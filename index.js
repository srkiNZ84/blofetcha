...
-v, --version <tag>          Find code on specific version (default: last)
-d, --dump                   Parse stack-trace dump from stdin.
...
(async(argv) => {
    ...
    if (argv.ln > 0) {
        ...
        if (argv.ver !== 'last' && String(version).split('-')[1] !== argv.ver) {
            ...
            if (version === config.last) {
                stderr(`Version ${argv.ver} not found, using last.`);
            }
        }

        const file = FS.read(path.join(config.ARCHIVE_PATH, version, `${argv.domain}.${argv.file}.js`));
        if (file) {
            const n = argv.ln - 1;
            const c = Math.max(0, n - argv.bc);
            const pad = Array(6).join(' ');

            file.split('\n')
                .slice(c, 1 + c + argv.bc + argv.ac)
                .forEach((x, i) => {
                    stdout(`${(c + i === n ? '\x1b[31m' : '') + (pad + (c + i + 1)).slice(-6)}  ${x}\x1b[0m`);
                });
        }
        else {
            process.exit(1);
